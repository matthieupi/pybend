"""Tests for RemoteMatrix HTTP(S) reference adapter."""
...
class TestRemoteMatrix:
    ...
    def test_can_handle_absolute_http_entity_and_class_targets(self):
        adapter = RemoteMatrix(remotes={'storage': 'http://storage:7100'})
        assert adapter.can_handle(TX(name='get', source='test', target='http://storage:7100/File/12')) is True
        assert adapter.can_handle(TX(name='reindex', source='test', target='http://storage:7100/File')) is True
        assert adapter.can_handle(TX(name='get', source='test', target='/File/12')) is False
        assert adapter.can_handle(TX(name='get', source='test', target='https://example.com/File/12')) is True
        assert adapter.can_handle(TX(name='get', source='test', target='n3tx://storage/File/12')) is False

    def test_url_mapping_for_get_and_methods(self):
        adapter = RemoteMatrix(remotes={'storage': 'http://storage:7100/'})
        assert adapter._url_for(TX(name='get', source='test', target='http://storage:7100/File/12')) == 'http://storage:7100/File/12'
        assert adapter._url_for(TX(name='process', source='test', target='http://storage:7100/File/12')) == 'http://storage:7100/File/12/process'
    ...
    def test_headers_include_service_token_and_user_context(self):
        ...
        tx = TX(
            name='get',
            source='test',
            target='http://storage:7100/File/12',
    ...
    async def test_send_replies_to_matrix_pending_request(self):
        ...
        response = await matrix.request(
            TX(name='get', source='test', target='http://storage:7100/File/12'),
            timeout=1,
        )

        assert response.is_error is False
        assert response.data == {'$id': 'http://storage:7100/File/12', 'name': 'remote-file'}
    ...
    async def test_stream_parses_remote_sse_tx_envelopes(self):
        ...
        tx = TX(
            name='generate',
            source='app',
            target='http://compute:7200/ComputePipeline/0',
            data={'prompt': 'hi'},
            meta={'stream': True},
        )
    ...
    async def test_stream_uses_schema_route_for_class_method(self):
        ...
        tx = TX(
            ...
            source='app',
            target='http://compute:7200/ComputePipeline',
            data={'run_id': 'run-1'},
    ...
    async def test_stream_yields_error_tx_for_remote_error_frame(self):
        adapter = RemoteMatrix(remotes={'compute': 'http://compute:7200'})
        tx = TX(name='generate', source='app', target='http://compute:7200/ComputePipeline/0')
...
class TestMatrixReferenceResolver:
    ...
    async def test_resolves_through_matrix_remote_adapter(self):
        ...
        result = resolver.resolve('http://storage:7100/File/12')

        assert result == {'$id': 'http://storage:7100/File/12', 'name': 'remote-file'}
    ...
    async def test_raises_for_remote_errors(self):
        ...
        with pytest.raises(RuntimeError, match='No route'):
            resolver.resolve('http://storage:7100/File/12')
