...
Internal address resolution is available through `File.resolve(address)`:
...
When the payload contains `{"audio": "/File/1"}`, direct and actor routing
resolve that address into an authorized `File` instance before invoking the
...
`File` metadata and addresses, not provider-specific handles. The test suite
