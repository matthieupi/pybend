...
describe('ntx-sidebar.js (NTTSidebar)', () => {
  ...
  describe('constructor', () => {
    ...
    it('should create stylesheet link in shadow DOM', () => {
      ...
      expect(link.getAttribute('href')).toContain('ntx-sidebar.css');
    });
  });
  ...
  describe('VIEW_TAGS static mapping', () => {
    it('should map grid to ntx-list', () => {
      const Ctor = customElements.get('ntx-sidebar');
      expect(Ctor.VIEW_TAGS.grid).toBe('ntx-list');
    });

    it('should map table to ntx-table', () => {
      const Ctor = customElements.get('ntx-sidebar');
      expect(Ctor.VIEW_TAGS.table).toBe('ntx-table');
    });
  });
  ...
  describe('route templates from children', () => {
    it('should derive models from children with model attribute when no models attribute', () => {
      ...
      const child1 = document.createElement('ntx-table');
      child1.setAttribute('model', 'Grant');
      el.appendChild(child1);

      const child2 = document.createElement('ntx-list');
