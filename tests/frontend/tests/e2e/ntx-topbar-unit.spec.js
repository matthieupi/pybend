...
test.describe('ntx-topbar — Anonymous State', () => {

  test('topbar renders with .topbar nav and brand', async ({ page }) => {
    await gotoAnonymous(page);

    const structure = await page.locator('ntx-topbar').evaluate((el) => {
  ...
  test('signin-link present with correct text and href', async ({ page }) => {
    ...
    const signIn = await page.locator('ntx-topbar').evaluate((el) => {
  ...
  test('user-pill, user-dropdown, favorites, profile are NOT present', async ({ page }) => {
    ...
    const absent = await page.locator('ntx-topbar').evaluate((el) => {
      const sr = el.shadowRoot;
      return {
...
test.describe('ntx-topbar — Authenticated State', () => {
  ...
  test('dropdown shows correct email per user (bob)', async ({ page }) => {
    ...
    const info = await page.locator('ntx-topbar').evaluate((el) => {
      return {
