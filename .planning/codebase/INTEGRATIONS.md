...
- Ollama (local) — default LLM backend for agent execution
  ...
  - Entry point: `src/pybend/core/agents/mixin.py` → `AgentMixin.agent_run()`
...
- SQLite (stdlib `sqlite3` module)
  - Connection: file path via `PYBEND_SQLITE_DB` env var or constructor arg
  - Client: `src/pybend/core/storage/sqlite_storage.py` → `SQLiteStorage`
  - Interface: `src/pybend/core/storage/abstract_storage.py` → `AbstractStorage` (ABC)
...
**Schema Migrations:**
- Auto-migration: `src/pybend/core/storage/sqlite_migration.py` → `SQLiteMigration`
...
**Auth Provider:**
- Custom JWT-based authentication (standalone `authorize` package — zero PyBend imports)
  - Package: `src/pybend/core/authorize/`
  - JWT: PyJWT with HS256 algorithm (`src/pybend/core/authorize/auth.py`)
  - Password hashing: bcrypt (`src/pybend/core/authorize/auth.py`)
  - Token payload: `{user_id, email, role, exp, iat}`
  - Token header: `x-access-token` (extracted by `JWTAuthMiddleware` in `src/pybend/core/api/backend.py`)
  - Token endpoints: `POST /users/login`, `POST /users/register` (from `BaseUser` model)

**Authorization (ABAC):**
- Rule-based access control: `src/pybend/core/authorize/rules.py`
...
- Resolver: `src/pybend/core/authorize/resolver.py` → `DefaultResolver`
...
- Tier 1: `src/pybend/core/api/auth_interceptor.py` — fast gate at protocol boundary
...
- Tier 2: `src/pybend/core/models/actor_model.py` → `ActorModel._authorize()`
  - Full ABAC with resource instance (handles OWNER checks after DB fetch)

## Internal Integration Points

**Actor System ↔ HTTP Routes:**
- Level 1/2 (direct): `src/pybend/core/api/routes_fastapi.py` → calls StorableMixin methods directly
- Level 3 (actor): `src/pybend/core/api/network_api.py` → `NetworkAPI` adapter
...
- Agent execution: `src/pybend/core/agents/mixin.py` → `AgentMixin.agent_run()`
  1. Creates transient `NetworkAdapter` per run (unique address: `_agent_{uuid}`)
  2. Registers adapter with Matrix root
  3. Discovers tools from actor addresses via `src/pybend/core/agents/tools.py` → `discover_tools()`
...
- Pipeline: `src/pybend/core/models/proto_schema.py` — composable stages
  ...
  - Agent extension: `src/pybend/core/agents/schema_ext.py` → injects `agent` section
  - Widget extension: `src/pybend/core/widgets/schema_ext.py` → injects `ui.widget` + `ui.config`
...
- Registration flow in `src/pybend/core/utils/registrar.py`:
...
- Join model generation: `generate_join_model(parent, child)` in `src/pybend/core/models/proto_model.py`
...
- `src/pybend/core/models/storable_mixin.py` → injected into models with `__storable__ = True`
...
- `src/pybend/core/agents/mixin.py` → injected into models with `__agent__ = True`
- Provides: `agent_run(prompt, tools, task, user, **kwargs)`
- Only `AgentActor` currently uses this (`src/pybend/core/agents/actor.py`)

## Protocol Adapters (NetworkAdapter hierarchy)

All adapters extend `src/pybend/core/api/network_adapter.py` → `NetworkAdapter(Actor)`:

**NetworkAPI** — HTTP REST adapter (`src/pybend/core/api/network_api.py`)
- Bridges FastAPI routes → Matrix TX messages
- Address: `api`
- Used in Level 3 routing mode

**NetworkWebSocket** — WebSocket adapter (`src/pybend/core/api/network_ws.py`)
...
**NetworkMCP** — MCP JSON-RPC 2.0 adapter (`src/pybend/core/api/network_mcp.py`)
...
**NetworkAP** — ActivityPub federation adapter (`src/pybend/core/api/network_ap.py`)
...
**Service Discovery:**
- `GET /_meta` — model registry, capabilities, health (`src/pybend/core/api/discovery.py`)
...
- `MethodError` exception class (`src/pybend/core/utils/erroring.py`) for custom method errors
- `TX.from_exception()` centralizes exception → error TX mapping with semantic HTTP codes

**Logs:**
- Python `logging` module throughout
- Logger hierarchy: `pybend.models`, `pybend.actors`, `pybend.storage`, `pybend.agents`, `pybend.network`, `pybend.network.api`, `pybend.network.ws`, `pybend.network.mcp`, `pybend.network.ap`, `pybend.schema`, `pybend.api`, `pybend.authorize`
...
- `PYBEND_JWT_SECRET` — must be changed from default (framework warns if insecure)
- `PYBEND_SQLITE_DB` — database file path
- `ANTHROPIC_API_KEY` — if using Anthropic LLM provider for agents

**Optional env vars:**
- `PYBEND_HOST`, `PYBEND_PORT`, `PYBEND_API_URL` — network config
- `PYBEND_DEBUG` — controls OpenAPI docs visibility and error detail
- `PYBEND_SSR` — server-side rendering mode
- `PYBEND_JWT_EXPIRY_HOURS` — token lifetime
- `GENERATE_DOCS` — set to `false` to skip auto-doc generation (used in tests)

**Secrets location:**
- Environment variables (no `.env` file committed)
- Dev defaults in `src/pybend/core/config.py` and `src/pybend/core/authorize/auth.py`
- Insecure secret detection: `src/pybend/core/authorize/auth.py` → `INSECURE_SECRETS` set
