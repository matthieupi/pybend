...
**Secondary:**
- SQL (SQLite dialect) — schema creation, migrations, queries in `src/n3tx/core/storage/sqlite_storage.py`
- HTML/CSS — static frontend pages in `src/n3tx/static/` and `example_grants/static/`
...
- npm (frontend tests only, `src/n3tx/static/package.json`)
- Lockfile: `package-lock.json` present for frontend; no `requirements.txt` lock — dependencies specified in `pyproject.toml`

## Frameworks

**Core:**
- FastAPI 0.129.0 (spec: `>=0.115,<1.0`) — ASGI web framework, auto OpenAPI docs. Entry: `src/n3tx/core/api/backend.py`
- Pydantic 2.12.5 (spec: `>=2.7,<3.0`) — model validation, JSON Schema generation, BaseModel inheritance. Entry: `src/n3tx/core/models/proto_model.py`
- pydantic-ai 1.64.0 (spec: `>=1.0`, optional `[agents]` extra) — LLM agent framework. Entry: `src/n3tx/core/agents/mixin.py`
- Uvicorn 0.41.0 (spec: `>=0.34,<1.0`) — ASGI server

**Testing:**
- pytest 8.4.2 (spec: `>=8.2`) — Python test runner. Config: `src/n3tx/core/pytest.ini`
...
- vitest 1.6.0 — frontend JS unit tests. Config: `src/n3tx/static/vitest.config.js`
- Playwright 1.44.0 — frontend E2E tests. Config: `src/n3tx/static/tests/e2e/playwright.config.js`

**Build/Dev:**
- setuptools >=68.0 — Python build backend (`pyproject.toml`)
- pyinstrument >=4.6 — performance profiling (dev dependency)
- esbuild — JS bundling for SSR mode (installed in `src/n3tx/static/node_modules/`)
...
- `PyJWT>=2.8.0` (installed: 2.11.0) — JWT token creation/verification in `src/n3tx/core/authorize/auth.py`. Algorithm: HS256.
- `bcrypt>=4.0.0` (installed: 5.0.0) — password hashing in `src/n3tx/core/authorize/auth.py`
...
- `Flask>=3.1` + `flasgger>=0.9` — alternative Flask backend (optional `[flask]` extra), route layer in `src/n3tx/core/api/routes_flask.py`

## Configuration

**Environment:**
- All configuration via `N3TX_*` environment variables, with defaults in `src/n3tx/core/config.py`
- Key env vars:
  - `N3TX_HOST` (default: `0.0.0.0`)
  - `N3TX_PORT` (default: `5000`)
  - `N3TX_SQLITE_DB` (default: `n3tx.db`, grants app uses `grants.db`)
  - `N3TX_JWT_SECRET` (default: dev secret — warns if insecure)
  - `N3TX_JWT_EXPIRY_HOURS` (default: `24`)
  - `N3TX_DEBUG` (default: `false` — controls OpenAPI docs visibility, error detail)
  - `N3TX_SSR` (default: `off` — modes: `off`, `schema`, `bundle`, `full`)
  - `N3TX_API_URL` (default: `http://localhost:{PORT}`)
  - `N3TX_BACKEND` (default: `fastapi`)
...
- `src/n3tx/static/package.json` — frontend test dependencies only (no build step for production)
- `src/n3tx/static/vitest.config.js` — frontend test configuration (jsdom environment)

## Database

**SQLite:**
- Default storage backend. Only backend currently supported.
- Connection pooling via `queue.Queue` in `src/n3tx/core/storage/sqlite_storage.py`
- WAL mode enabled by default for concurrent read/write
- `PRAGMA busy_timeout=5000` for lock contention
- Auto-migration on startup via `src/n3tx/core/storage/sqlite_migration.py`
