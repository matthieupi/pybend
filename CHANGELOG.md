...
## Backend
- [x] **[Performance Optimizations](src/n3tx/core/docs/changelogs/03_Performance_Backend.md)**
...
- [x] **User model UI hints** — added `__ui__` with `renderer` config pointing to `ntx-user` component
...
- [x] **[Authorization & Authentication](src/n3tx/docs/AUTHORIZATION.md)** — standalone `authorize/` package
  ...
  - `configure()` function for JWT settings — package has zero N3TX imports, reads env vars by default
...
- [x] **[ForeignKey Hydration](src/n3tx/core/docs/features/01_ForeignKey_Hydration.md)**
...
## Frontend
- [x] **[Performance Optimizations](src/n3tx/static/docs/changelogs/04_Performance_Frontend.md)**
...
- [x] **`<ntx-method>` button layout** — new compact icon + count pill rendering for social actions (like, favorite)
  ...
  - Click handler routes through N3TX Actor with `_response_` inbox for auto-refresh
...
- [x] **`<ntx-favorites>` component** — simple page wrapper mounting `<ntx-list model="ProductLike">` at `#@favorites` route
- [x] **Topbar Favorites link** — authenticated users see a "Favorites" nav link in `<ntx-topbar>`
  - `topbar-nav` CSS block with hover transitions
- [x] **JSON render depth guard** — `ntx-logs.js` caps recursion at depth 8 to prevent stack overflow on circular structures
...
- [x] **Resource-aware OWNER checks** — `Permissions.canAction()` now accepts entity data for real OWNER evaluation
  ...
  - All `canAction()` call sites in `ntx-item.js` pass `this.value` for per-entity checks
...
- [x] **Delete support** — delete button in `ntx-item` `md()` with confirmation dialog and ABAC check
  - `N3TX.js` DynamicClass implements DELETE handler: removes instance, notifies watchers
- [x] **`<ntx-user>` component** — custom NTTItem subclass with circular avatar rendering (xs/sm)
  - Fallback to ui-avatars.com when no image is set
  - User model `__ui__.renderer` wires it automatically
- [x] **ATTACH routing improvements** — `N3TX.js` fetches individual entities on ATTACH for non-existent instances
...
- [x] **`ntx-item` sm display refinements** — respects `field_order`, renders `$ref` fields as leading avatars
...
- [x] **Kitchen sink page** — complete rewrite of `schema.html` into a full component showcase
  ...
  - Live `ntx-list` + `ntx-router` with click-to-navigate
...
- [x] **[Refactor PTT into N3TX](src/n3tx/static/docs/PTT_N3TX_MERGE.md)**
  - Merged PTT into N3TX — N3TX is now the universal type registry, schema proxy, and ATTACH router
  - Added null-pointer bootstrap pattern with TX message queueing for async schema loading
  - Added `N3TX.SCHEMA` static handler for bootstrap completion and DynamicClass generation
  - Added `N3TX.ATTACH` universal router handling both type-level and instance-level ATTACHes
  - Added optimistic update flow: `N3TX.UPDATE` updates locally then forwards to backend
- Added `save()` method on `<ntx-item>` with N3TX-mediated update path
- Added favicon (SVG)
...
- Updated `COMPONENTS.md` with ntx-user, delete button, and ListElement pagination docs
...
- Created comprehensive README.md for N3TX 0.6 (quickstart, architecture, dataflow, CRUD, components)
- Documented FK hydration across README.md, ARCHITECTURE.md, ACTORS.md, COMPONENTS.md, MESSAGE_PROTOCOL.md, TRANSPORT.md
- Added PTT/N3TX merge report (`docs/PTT_N3TX_MERGE.md`)
- Added FK hydration implementation report (`core/docs/features/FK_HYDRATION.md`)
- Created TODOs.md for project task tracking

# 0.6.0 (2025-12)

## Frontend — Actor Model Architecture (N3TXTX v0.6)
- Rewrote entire frontend from event-bus architecture (N3TX 0.5) to actor-based model
...
- Introduced `N3TX` (Named Transfer Type) as per-entity instance base class
- Introduced `DynamicClass` — runtime-generated N3TX subclass per backend model with typed getters/setters
...
## Frontend — N3TXTX v0.5
- Created N3TX 0.5 frontend with event-based architecture (`Event.js`, `Remote.js`, `HTTP.js`)
- Created `<ntx-element>` web component — base component with data binding and render lifecycle
- Created `<ntx-list>` — renders a collection of `<ntx-item>` elements from a model type
- Created `<ntx-item>` — renders a single entity with auto-generated form fields from schema
- Created `<ntx-method>` — invokes backend RPC methods on entities
- Abstracted `<ntx-element>` base class, improved cross-system bindings
- Created `Formidable` form generator — auto-generates input fields from JSON schema (text, number, select, textarea, ref)
- Uncoupled form generation from `<ntx-item>` into standalone generator
- Added `dark-theme.css` with design tokens
- Removed legacy N3TX 0.0–0.4 frontend versions (~10,000 lines deleted)
...
## Initial release
- Created n3tx module with flask and swagger support
