...
This command is inspired by `/bugfix`, but it is different in one crucial way:
...
Never skip Phase 2. The contract triage gate is the entire point of this command.
...
Do not proceed until the user approves the target.
...
- Fix fixture scope, setup/teardown, mocks, environment, timing, or global state.
...
If the approved fix fails the narrow verification and contradicts your theory,
revert the attempted changes and return to Phase 2 or Phase 3.
