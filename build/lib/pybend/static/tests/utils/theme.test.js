...
describe('theme.js', () => {
  ...
  beforeEach(() => {
    window.localStorage.removeItem('ntt-theme');
  ...
  it('getTheme() should return stored theme from localStorage', async () => {
    const { getTheme } = await import('../../utils/theme.js');
    window.localStorage.setItem('ntt-theme', 'light');
  ...
  it('setTheme() should store theme in localStorage', async () => {
    ...
    setTheme('light');
    expect(window.localStorage.getItem('ntt-theme')).toBe('light');
  });
  ...
  it('toggleTheme() should switch from dark to light', async () => {
    ...
    window.localStorage.setItem('ntt-theme', 'dark');
    toggleTheme();
    expect(getTheme()).toBe('light');
  });
