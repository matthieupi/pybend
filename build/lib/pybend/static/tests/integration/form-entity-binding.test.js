...
describe('Form Generation', () => {
  ...
  it('field_order from schema.ui is respected', () => {
    ...
    const mainPos = html.indexOf('ntt-group-main');
    const socialPos = html.indexOf('ntt-group-Social');
  ...
  it('attached methods render ntt-method elements after target field', () => {
    const ntt = makeNtt();
    const attached = {
  ...
  it('getListInput resolves child tag from $defs renderer hints', () => {
    ...
    const html = Formidable.getListInput(ntt, 'comments', 'display');
    // Default childTag from renderer config or ntt-item
