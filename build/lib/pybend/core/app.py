...
class PyBendApp:
    ...
    def build(
        ...
        # 6. Mount app-specific static directories if provided
        if self._static_dirs:
            from fastapi.staticfiles import StaticFiles

            for idx, static_dir in enumerate(self._static_dirs):
                mount_name = f"app_static_{idx}" if idx > 0 else "app_static"
                backend.app.mount(
                    f"/app-static" if idx == 0 else f"/app-static-{idx}",
