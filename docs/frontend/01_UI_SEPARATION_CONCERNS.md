# N3TX Component Layer: Separation of Concerns

Architectural analysis and refactoring plan for the N3TX frontend component hierarchy.

---

## 1. System Context

N3TX is a schema-driven frontend framework built on vanilla JS Web Components. The backend (N3TX/FastAPI) publishes JSON Schemas for every model. The frontend fetches these schemas at runtime, creates DynamicClasses (typed N3TX instances), and renders UI components that stay in sync via a message bus (Matrix/Actor system).

### The Data Layer (N3TX Core — unchanged by this refactor)

The data layer lives in `core/N3TX.js` and is separate from the component layer. It handles:

- **Schema registry**: `N3TX.ATTACH` bootstraps models on first reference — fetches schema from backend, creates a `DynamicClass` via `prototype()`, stores it in a global `#prototypes` map.
- **DynamicClass**: A runtime N3TX subclass per model (e.g., "Product"). Holds the schema, typed property accessors, instance map, CRUD methods, watcher set. Created by the `prototype()` factory function.
- **Instance management**: Each backend record becomes a `DynamicClass` instance (an N3TX actor) with its own address, href, value, and signal/observe reactivity.
...
HTMLElement
  └── Component              (core/Component.js)
        └── NTTElement       (components/ntx-element.js)
              ├── Item       (components/ntx-item.js)     -> <ntx-item>
              └── List       (components/ntx-list.js)     -> <ntx-list>
```
...
**Problems:**
- Circular import: line 1 imports `'../components/ntx-item.js'` (a grandchild)
...
- Ref resolution: `ref` attr -> READ TX (URL) or ATTACH TX (N3TX address)
...
- `attach(addr)` — N3TX.attach wrapper
...
### Item (`components/ntx-item.js`, ~165 lines)

Extends NTTElement. Registered as `<ntx-item>`.
...
- Method rendering: iterates `schema.methods`, stamps `<ntx-method>` elements
...
### List (`components/ntx-list.js`, ~72 lines)

Extends NTTElement. Registered as `<ntx-list>`.
...
- Rendering: header with model name + count, stamps `<ntx-item>` per address

**Problems:**
- Hardcodes `<ntx-item>` as child element — no way to customize
...
| # | Issue | Where | Impact |
|---|---|---|---|
| 1 | Circular import | Component.js:1 imports ntx-item.js | Module loading fragility |
| 2 | Component has zero independent consumers | Component.js | Unnecessary abstraction layer |
| 3 | Double `Actor.subclass` | ntx-list.js:71 | Inconsistent (Item doesn't do it) |
...
| 11 | Edit UX baked into Item | ntx-item.js | Can't extend without inheriting edit toggle |
| 12 | List hardcodes `<ntx-item>` children | ntx-list.js | No customization |
...
HTMLElement
  └── Component                 (merged Component + NTTElement)
        ...
        └── ListElement         (collection lifecycle)
              └── NTTList       (built-in default: grid of <ntx-item>)
```
...
- Message handlers:
  ...
  - `DESCRIBE(data)` — receives proto + data together (from N3TX instance ATTACH response)
...
- `get childTag()` — overridable hook, defaults to `'ntx-item'`
...
| Concern | How |
|---|---|
| Schema fetching & parsing | N3TX.ATTACH -> SCHEMA TX -> DynamicClass creation |
| Data fetching (CRUD) | DynamicClass.call('READ') via NetworkAdapter |
| Instance lifecycle | DynamicClass.READ creates/updates N3TX instances |
...
| Fetch trigger | Receives push (DESCRIBE from N3TX instance) | Actively pulls (triggers collection READ) |
...
| `components/ntx-element.js` | `core/Component.js` (new base) | Remove `describe()`, `update()`, auto-render from setter. Add `get styles()` hook. Absorb old Component.js internals. |
| `components/ntx-item.js` | Split: `components/NTTElement.js` (entity base) + `components/ntx-item.js` (default) | Data handlers -> NTTElement. Formidable + edit UX -> NTTItem. |
| `components/ntx-list.js` | Split: `components/ListElement.js` (collection base) + `components/ntx-list.js` (default) | Add `get childTag()`. NTTList becomes a thin shell. |

### What does NOT change

- `core/N3TX.js` — Data layer, DynamicClass, schema registry. Untouched.
...
- `components/ntx-method.js` — Method buttons. Untouched.

### Cleanup included in migration

- Remove circular import (Component.js -> ntx-item.js)
...
components/
  ...
  ntx-item.js         REFACTORED - thin default (extends NTTElement)
  ntx-list.js         REFACTORED - thin default (extends ListElement)
  ntx-element.js      DELETED - absorbed into core/Component.js
  ntx-method.js       unchanged

generators/
  form.js             unchanged (Formidable, consumed by ntx-item.js)
```
