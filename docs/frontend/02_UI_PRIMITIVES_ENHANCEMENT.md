# N3TX UI Primitives Enhancement Plan

A comprehensive plan to evolve the N3TX frontend component layer into a developer-friendly framework that provides composable, schema-driven primitives — making tools like Redux, fetch(), React, and GraphQL redundant for most use cases.
...
| `src/n3tx/core/main.py` | App entry: registers models, starts server |

### Key Frontend Files

| File | Role |
|---|---|
| `static/core/N3TX.js` | Core entity system. `N3TX` class = schema registry + instance manager. `prototype()` factory creates DynamicClasses from schema. |
...
| `static/components/ntx-item.js` | Built-in default item (extends NTTElement). Formidable auto-render + edit toggle. |
| `static/components/ntx-list.js` | Built-in default list (extends ListElement). Zero-config grid of children. |
| `static/components/ntx-method.js` | Renders callable methods as buttons. Unchanged. |
...
```python
# src/n3tx/example/models/product.py
...
```
1. <ntx-list model="Product">           Component appears in DOM
2. attributeChangedCallback('model')     Sends ATTACH TX → target: 'N3TX'
3. N3TX.ATTACH("Product")                Static handler checks #prototypes registry:
...
5. N3TX.SCHEMA(data)                     Creates DynamicClass via prototype()
   - Registers nested $defs models
   - Replays queued ATTACHes
   - Triggers DynamicClass.call('READ')  → GET /products
6. DynamicClass.READ(data)              Creates N3TX instances per record
   - Notifies all watchers with instance addresses
7. List.UPDATE([addrs])                 Receives address array, renders grid
8. <ntx-item ref="Product/1">           Stamped per address
9. N3TX instance.ATTACH(tx)             Instance responds with DESCRIBE TX
...
HTMLElement
  └── Component              (core/Component.js, ~70 lines)
        └── NTTElement       (components/ntx-element.js, ~155 lines)
              ├── Item       (components/ntx-item.js, ~165 lines)  → <ntx-item>
              └── List       (components/ntx-list.js, ~72 lines)   → <ntx-list>
```
...
**Problem**: Has zero independent consumers — every Component is always an NTTElement. Circular import (line 1: `import '../components/ntx-item.js'`).

### NTTElement (`components/ntx-element.js`)
...
export class NTTElement extends Component {
  ...
  // ref setter: sends READ TX (URL) or ATTACH TX (N3TX address)
  set ref(href) { ... }

  // Called by N3TX core when DynamicClass prototype is ready
...
### List (`components/ntx-list.js`)

Handles collection: subscribes to DynamicClass watcher, triggers READ, receives address array, stamps `<ntx-item>` per address. Has redundant `Actor.subclass(List)`.

### Identified Issues (Full List)

1. Circular import: Component.js imports ntx-item.js (grandchild)
...
12. List hardcodes `<ntx-item>` as child — no customization
...
- Schema resolution: `model` attr → ATTACH TX → N3TX core → `define(proto)` → `definedCallback()`
- Ref resolution: `ref` attr → ATTACH TX (N3TX address) or READ TX (URL)
...
- `attach(addr)` — imperative N3TX.attach convenience (lazy-imported)
...
- `get childTag()` — overridable, defaults to `'ntx-item'`

### Built-in Defaults

```js
// components/ntx-item.js — zero-config single entity
...
// components/ntx-list.js — zero-config collection
...
| `components/ntx-item.js` | **Slimmed** — thin NTTItem default over NTTElement | 115 |
| `components/ntx-list.js` | **Slimmed** — thin NTTList default over ListElement | 19 |
| `components/ntx-element.js` | **Deleted** — absorbed into `core/Component.js` | - |
| `generators/form.js` | **Enhanced** — field_order, widget, groups, validation (§4, §11) | 268 |

### Issues Resolved

1. Circular import (Component → ntx-item) — **gone**
...
9. Hardcoded `<ntx-item>` in List — **`childTag` + `createChild()` + template support in ListElement**

### Unchanged Files

`core/Matrix.js`, `core/Actor.js`, `core/Observable.js`, `core/TX.js`, `components/ntx-method.js` — the actor system and message infrastructure remain untouched. `core/N3TX.js` was modified for SSR pre-loading (§7). `generators/form.js` was enhanced with UI hint consumption, field ordering, widget mapping, grouped rendering (§4, §11), schema-aware relationship rendering (§13), and field-level access control (§12). `utils/Permissions.js` was added for frontend permissions (§12).
...
**`groups`**: When `schema.ui.groups` is defined, fields are wrapped in `<fieldset class="ntx-group ntx-group-{name}">` with `<legend>` per group. Fields not belonging to any group render ungrouped at the end. New helper: `renderGroupedFields(ntt, renderableFields, groups, mode)`.
...
get childTag() {
  return this.schema?.ui?.renderer?.item || 'ntx-item';
}
...
```html
<script type="application/json" data-ntx-schema="Product">
  {{ product_schema_json }}
</script>
<script type="application/json" data-ntx-data="products">
  {{ products_json }}
</script>
<ntx-list model="Product"></ntx-list>
```
...
Two private static helper methods added to `N3TX` class in `core/N3TX.js`:

- **`#consumePreloadedSchema(model)`** — Checks `document.querySelector('script[data-ntx-schema="ModelName"]')`. If found, parses JSON, feeds to `N3TX.SCHEMA()` directly, removes the script tag (one-shot consumption). Returns `true` if pre-loaded schema was found.
- **`#consumePreloadedData(tablename)`** — Checks `document.querySelector('script[data-ntx-data="tablename"]')`. If found, parses JSON, removes script tag, returns data array. Returns `null` if not found.

**Wired into three bootstrap paths:**

1. `N3TX.attach()` — "Never seen" branch: tries `#consumePreloadedSchema(addr)` before sending SCHEMA TX
2. `N3TX.ATTACH()` — "Never seen" branch: same pre-load check before network fetch
3. `N3TX.SCHEMA()` — After creating DynamicClass: tries `#consumePreloadedData(tablename)` and calls `DC.READ(data)` directly instead of `DC.call('READ')` if found

**Queue-then-consume pattern**: Both `attach()` and `ATTACH()` set up the waiting queue *before* calling `#consumePreloadedSchema()`. This ensures that when `N3TX.SCHEMA()` runs synchronously and replays the queue, all pending callbacks/TXs are already queued.
...
Eliminates both network round-trips for server-rendered pages. For SPA-style navigation (no server page render), the existing fetch flow works unchanged. Only `core/N3TX.js` was modified — no changes to the component layer.
...
A single component renders differently based on the space its parent allocates, not the screen size. This makes components truly context-aware — the same `<ntx-item>` works in a dashboard grid, a detail page, a sidebar, or an inline mention.
...
```html
<ntx-item display="xs"></ntx-item>    <!-- abstract name -->
<ntx-item display="pill"></ntx-item>  <!-- semantic alias -->
<ntx-item display="auto"></ntx-item>  <!-- back to auto -->
```
...
**CSS scoping**: `.card[data-display="xs"]`, `.card[data-display="sm"]`, etc. Each size has distinct layout rules in `ntx-item.css`. Host-level: `:host([display="xs"]) { display: inline-block; }`.
...
```html
<ntx-list model="Product" item-display="sm"></ntx-list>
```
...
**File**: `src/n3tx/core/utils/scaffold.py`
...
**NTTItem integration** (`components/ntx-item.js`):
...
1. **Schema-aware child tag resolution**: Looks up the referenced model's `$defs` entry for `ui.renderer.item`. If `Comment` has `renderer.item: 'comment-bubble'`, the list field stamps `<comment-bubble>` instead of `<ntx-item>`.
...
```js
// Resolution: $defs[modelName].ui.renderer.item → 'ntx-item' fallback
...
New CSS classes in `ntx-item.css`:
...
**Implementation points**: `core/N3TX.js` (pre-load check), `core/utils/scaffold.py` (CLI + API endpoint).
...
| ~~**P2**~~ | ~~SSR pre-loading~~ | ~~§7~~ | **DONE** | ~~`N3TX.js` change only~~ |
...
4. **Don't break existing.** `<ntx-list model="Product">` and `<ntx-item>` must continue to work throughout. The built-in defaults (NTTItem, NTTList) are thin shells over the new bases.

5. **Minimize changes to the data layer.** `N3TX.js`, `Matrix.js`, `Actor.js`, `Observable.js`, `TX.js` should remain untouched unless absolutely necessary. The only exception so far is the SSR pre-loading check in §7 (two private static helpers + three guard clauses in `N3TX.attach()`, `N3TX.ATTACH()`, and `N3TX.SCHEMA()`).
...
- Default components (`ntx-item.js`, `ntx-list.js`) are thin. They should be under ~50 lines each, delegating all logic to their base classes.

### Testing

- Start server: `cd /workspace/src/n3tx/core && python3 main.py`
...
- After component refactor, verify that `<ntx-list model="Product">` still renders correctly in the browser.
