...
Documentation for the web component layer that renders N3TX data in the browser.
...
Unified base class for all N3TX web components. Merges Actor bridge, Shadow DOM, schema resolution, adaptive display, and stylesheet management into a single class.
...
HTMLElement
  └── Component              (core/Component.js, ~317 lines)
        ...
        │     └── NTTItem    (components/ntx-item.js — built-in default)
        └── ListElement      (components/ListElement.js — collection lifecycle)
              └── NTTList    (components/ntx-list.js — built-in default)
```
...
| `model` | `string` | Model name (e.g., `"Product"`). Setting triggers ATTACH TX to N3TX. |
| `ref` | `string` | Entity reference. URL → READ TX. N3TX address → ATTACH TX. |
...
```html
<!-- Abstract size -->
<ntx-item display="sm"></ntx-item>

<!-- Semantic alias -->
<ntx-item display="pill"></ntx-item>

<!-- Programmatic -->
<script>
  document.querySelector('ntx-item').display = 'lg';
</script>

<!-- Back to auto -->
<ntx-item display="auto"></ntx-item>
```
...
2. attributeChangedCallback sends ATTACH TX → target: 'N3TX'
3. N3TX bootstraps DynamicClass (fetch schema, create prototype)
4. N3TX.attach callback calls component.define(DynamicClass)
...
| `attach(addr)` | Imperative `N3TX.attach(addr, this.define)`. Lazy-imports N3TX to avoid circular deps. |
...
| `DESCRIBE(data)` | N3TX instance ATTACH response | Sets `schema = data.proto`, `value = data.data`, renders. Also subscribes to entity signal for live updates — future value changes (e.g., after `pull()`) automatically re-render. |
...
2. N3TX resolves DynamicClass → Component.define(DC) → definedCallback()
3. definedCallback():
...
<ntx-list model="Product" item-display="card"></ntx-list>
```
...
```html
<!-- router attribute auto-set by ntx-router, or set manually -->
...
<ntx-item ref="Product/1"></ntx-item>

<!-- Forced size -->
<ntx-item ref="Product/1" display="xs"></ntx-item>
<ntx-item ref="Product/1" display="pill"></ntx-item>
```
...
| `#methodsHtml()` | Generates `<ntx-method>` HTML for all schema methods. |
...
```html
<ntx-method model="Product" uuid="1" method="comment" label="Add Comment"></ntx-method>
```
...
|-----------|-------------|
| `model` | Model name for N3TX lookup. |
...
Pure navigation state Actor. Manages route state, a history stack, and optional hash sync. No DOM — view management is the ntx-router's responsibility.
...
**File:** `components/ntx-router.js`
**Tag:** `<ntx-router>`
**CSS:** `components/ntx-router.css`
...
When ntx-router shows a dynamic view, the `<slot>` is removed from shadow DOM. But the light DOM children (`<ntx-list>`) stay alive — they keep their data, subscriptions, and state. Going back restores the `<slot>`, and the list re-projects instantly with no re-fetch or re-render.

### Tag Resolution (String Routes)

For string routes like `"Product/3"`, ntx-router resolves the component tag:
...
Fields are wrapped in `<fieldset class="ntx-group ntx-group-{name}">` with `<legend>` per group. Ungrouped fields render after all groups.
