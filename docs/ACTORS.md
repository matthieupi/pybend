...
with instances. N3TX's actor system doesn't. Every operation —
...
```python
from n3tx.core.actors import Actor, Matrix, TX
...
```python
from n3tx.core.actors.actor_proxy import ActorLike
...
```bash
# Run actor tests
cd src/n3tx/core && python3 -m pytest actors/tests/ -v

# Run network adapter tests
cd src/n3tx/core && python3 -m pytest api/tests/ -v

# Run all framework unit tests (includes actor tests from tests/unit/)
cd src/n3tx/core && python3 -m pytest tests/unit/ -q
```
...
```python
from n3tx.core.actors.tests.conftest import mock_method
