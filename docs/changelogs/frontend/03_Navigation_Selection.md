...
> **Status:** Complete.
> **Scope:** Added client-side navigation (Router actor + ntx-router view container), collection selection API, and fixed duplicate network requests.
...
3. **ntx-router** (`components/ntx-router.js`) — generic view container ("mini browser") that loads any component dynamically
...
| View container | None | Generic ntx-router (loads any component) |
| Collection selection | None | Set-based API on ListElement |
| Comment network requests | 4x per comment | 1x per comment |
| Files to edit for new views | N/A | 0 (ntx-router is generic) |
...
When ntx-router shows a dynamic view, the `<slot>` is removed from shadow DOM but light DOM children (`<ntx-list>`) stay alive — they keep data, subscriptions, and state. Going back restores the `<slot>`, and the list re-projects instantly. No re-fetch, no re-render.
...
Uses `window.N3TX.get(model)` for lazy access to avoid circular deps.
...
- Skips interactive elements: `button, input, textarea, select, a, ntx-method`.
...
`cursor: pointer` added to `.card[data-display="xs"]`, `.card[data-display="sm"]`, and `.card[data-display="md"]` in `ntx-item.css`.
...
Each product card re-rendered twice (DESCRIBE double-render), and each render created comment `ntx-item` elements that each fired 2 READ requests (double ref setter). Total: **4x network requests per comment**. After fix: **1x**.
...
| `components/ntx-router.js` | Generic view container (slot, mount, back) | ~141 |
| `components/ntx-router.css` | Router chrome styles (back button, title, content area) | ~59 |
...
| `components/ntx-item.js` | Added: TX import, card click handler in `#bindEvents()` |
| `components/ntx-item.css` | Added: `cursor: pointer` on xs, sm, md cards |
...
| `index.html` | Wrapped `<ntx-list>` in `<ntx-router name="main" hash>`, added router import |
...
1. Open `index.html` — product list renders normally (ntx-router shows slot, no chrome)
