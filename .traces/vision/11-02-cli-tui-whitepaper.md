...
The question is not whether PyBend should have a CLI. The question is what *kind* of CLI -- and the answer, as we will see, is fundamentally shaped by the framework's schema-driven architecture.
...
**Application to PyBend:** `ProtoModel.schema()` at `/workspace/src/pybend/core/models/proto_model.py` lines 197-316 encodes an extraordinary density of knowledge per model. A `pybend inspect Product` command that surfaces this knowledge in the terminal encodes at least six pieces: field definitions, validation constraints, access rules, method signatures, auto-generated routes, and UI configuration. That is twice the threshold.
...
This principle comes from observing how PyBend's web frontend already works. `NTT.SCHEMA()` at `/workspace/src/pybend/static/core/NTT.js` line 390 receives the schema from the backend and creates DynamicClasses with typed properties, methods, and reactive getters. `form.js` at `/workspace/src/pybend/static/generators/form.js` line 17 reads the same schema and generates HTML forms. `Permissions.js` reads the `access` section. `ntt-method.js` reads the `methods` section. Each consumer is independent. None knows about the others.

A CLI/TUI layer would be another independent consumer. No adapter pattern needed. No intermediate translation. The schema IS the abstraction layer. This is architecturally superior to RJSF's approach, which requires a separate `uiSchema` alongside the data schema -- PyBend embeds UI hints directly in `json_schema_extra`, so one schema serves all renderers.
...
This principle maps directly to PyBend's existing three-level bootstrap:

```
Level 1 (one-liner):    create_app(models=[Product], storage="sqlite:///app.db")
Level 2 (builder):      PyBendApp(storage=...).model(Product).join(Product, Comment).build()
...
```
Level 1 (zero-config):  pybend run              # finds main.py, starts server
Level 2 (flags):        pybend run --port 8080 --host 0.0.0.0 --workers 4
Level 3 (explicit):     pybend run --app myproject.main:app --storage sqlite:///custom.db
```
...
**Application to PyBend:** Keep the command count low. The research recommends 7 core commands for a framework at PyBend's maturity. Each command must encode enough framework knowledge to justify its maintenance cost. Commands that merely wrap shell operations (`pybend test` wrapping `pytest`) belong in a Makefile, not the CLI.
...
**Application to PyBend:** `pybend run` and `pybend inspect` will be used far more often than `pybend new`. Invest accordingly.
...
A CLI that reads this registry does not need per-model command classes. A single `pybend list <table>` command works for any registered model. A single `pybend inspect <Model>` command works for any model. The registry IS the command namespace.

### 3.2 `form.js` as the TUI Blueprint

The `Formidable` module at `/workspace/src/pybend/static/generators/form.js` is the most direct analog to a TUI form renderer. Its `getForm()` function (line 17) performs a sequence of operations that any form renderer must perform:
...
PyBend's frontend uses an Actor model (Matrix -> Actor -> NTT -> DynamicClass) where entities communicate via TX messages. The `Matrix` at `/workspace/src/pybend/static/core/Matrix.js` dispatches messages to local actors or remote endpoints. `NTT.ATTACH` (line 357) handles entity bootstrapping -- a component sends ATTACH, receives schema, creates DynamicClass, triggers READ.

This architecture is relevant for the TUI because Textual's own architecture mirrors it. Textual uses a message-based event system where widgets communicate via messages (`on_*` handlers). A Textual app's `compose()` yields child widgets (like Actor.spawn). Widget state changes propagate via `reactive()` attributes (like NTT's value setter triggering `signal()`). The mapping:

| PyBend Web | Textual TUI |
|-----------|-------------|
| `Matrix.dispatch(TX)` | `App.post_message(Message)` |
| `Actor.inbox(event)` | `Widget.on_event(event)` |
| `NTT.signal(callback)` | `reactive()` + `watch_*()` |
| `DynamicClass.instances` | Widget query: `self.query(EntityRow)` |
| `form.js.getForm(schema)` | `tui_form.build_form(schema)` |

This architectural resonance means a TUI admin would not fight PyBend's patterns -- it would extend them into a different rendering target.

### 3.4 The `PyBendApp.build()` Bottleneck

The single biggest architectural obstacle is that model registration is coupled to HTTP server creation. `PyBendApp.build()` at `/workspace/src/pybend/core/app.py` line 128 performs six steps in sequence:
...
```
$ pybend inspect Product --section access
...
PyBend's `scaffold.py` is schema-driven (reading from `ProtoModel.schema()`), not template-driven. This is architecturally sound. But a `pybend generate model` command that writes Python model files from templates would be template-driven and subject to drift. The correct approach for model creation is either (a) interactive prompts that output a minimal model file, or (b) no generation at all -- let developers write models by hand, using the example app and `pybend inspect` as references.

The exception is `pybend new <project>`, which generates an initial project skeleton. This is acceptable because it is used once, the templates are maintained as part of the framework, and the generated code uses `create_app()` which is the framework's own API.
...
Not every developer workflow needs a CLI command. `pybend test` would be a thin wrapper around `pytest`. `pybend deploy` would be a thin wrapper around deployment scripts. These commands carry no framework knowledge -- they are shell command wrappers. A Makefile or Justfile handles them with zero maintenance cost:
...
**Goal:** A working `pybend` CLI with 4 schema-aware commands.

**Decision gate:** Can a developer go from `pip install pybend` to inspecting a model schema in under 2 minutes?

| Deliverable | Effort | Depends On |
|-------------|--------|------------|
| `PyBendApp.setup()` extraction | 2-4 hrs | Nothing |
| `pybend` entry point + Typer app | 4-6 hrs | setup() |
| `pybend run` (uvicorn wrapper) | 2-3 hrs | Entry point |
| `pybend models` (list registered models) | 2-3 hrs | Entry point + setup() |
| `pybend inspect <Model>` (schema visualization) | 8-12 hrs | Entry point + setup() |
...
- Optional dependency: `pip install pybend[cli]`
...
| Deliverable | Effort | Depends On |
|-------------|--------|------------|
| `pybend shell` (REPL with models) | 4-8 hrs | Phase 1 |
| `pybend doctor` (diagnostics) | 4-6 hrs | Phase 1 |
| `pybend migrate:status` | 4-6 hrs | Phase 1 |
| `pybend seed` (wraps seed.py) | 2-4 hrs | Phase 1 |
| `pybend new <project>` (project scaffold) | 16-24 hrs | Phase 1 |
...
| `pybend create <Model>` (interactive form) | 4-6 hrs | tui_form |
| `pybend list <table>` (Rich table output) | 4-6 hrs | Phase 1 |

**Architectural decisions locked in Phase 3:**
- Textual as optional TUI dependency: `pip install pybend[tui]`
...
**Do not start Phase 4 until Phases 1-3 are stable and in use.** The TUI dashboard is a differentiator, not a necessity. Its value depends on PyBend's user base and their deployment patterns.

---

## 7. Conclusion

The CLI/TUI domain and PyBend's schema-driven architecture are not just compatible -- they are **mutually reinforcing**. The CLI world has spent 18 years converging on the insight that framework CLIs must encode domain knowledge, not just wrap shell commands. PyBend has spent its entire existence building a schema that encodes domain knowledge more richly than any comparable framework.
...
The end state is a framework where `pybend inspect Product` shows you the model's complete behavioral specification, `pybend create Product` launches a validated terminal form, `pybend shell` gives you a REPL with every model pre-loaded, and `pybend admin` gives you a full admin interface over SSH -- all derived from the same Python class definition that drives the web UI. This is not incremental improvement. This is what "the model is the app" means when taken to its logical conclusion.
...
### PyBend Codebase (Referenced Files)
24. `/workspace/src/pybend/core/models/proto_model.py` -- ProtoModel.schema(), _AUTO_HIDE_FIELDS, generate_join_model()
25. `/workspace/src/pybend/core/app.py` -- PyBendApp.build(), create_app(), _resolve_storage()
26. `/workspace/src/pybend/core/utils/registrar.py` -- registered_models, register_model()
27. `/workspace/src/pybend/core/api/routes_fastapi.py` -- register_routes(), route factories
28. `/workspace/src/pybend/static/generators/form.js` -- Formidable.getForm(), getInput(), validationAttrs()
29. `/workspace/src/pybend/static/core/NTT.js` -- NTT.SCHEMA(), prototype(), DynamicClass creation
30. `/workspace/src/pybend/static/core/Matrix.js` -- Message dispatch, NetworkAdapter bridge
31. `/workspace/src/pybend/static/core/Actor.js` -- Actor model, subclass(), register()
32. `/workspace/src/pybend/core/utils/decorators.py` -- @expose_route()
33. `/workspace/src/pybend/__init__.py` -- Public API, __version__
