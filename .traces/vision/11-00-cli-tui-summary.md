# CLI and TUI for N3TX: Executive Summary
...
| 2 | N3TX is **~70% of the way to a complete CLI** with zero new code -- model registry, schema generation, CRUD layer, migration system, scaffold generator all exist | The investment is primarily wiring, not invention |
| 3 | **Typer** (19K stars, 66M monthly PyPI downloads, same author as FastAPI) is the clear framework choice | Natural fit for N3TX's type-hint philosophy; zero ecosystem risk |
...
| 7 | Schema-driven generation (Prisma model) ages better than template-based scaffolding (Rails/CRA model) -- N3TX's `scaffold.py` already follows the correct pattern | N3TX's architecture is naturally resistant to the "scaffold trap" that killed CRA |
...
Of the 18 potential CLI commands identified in the analysis, **14 are direct wrappers** around functions that already exist in the codebase. Only four require genuinely new functionality: project creation (`n3tx new`), model file generation (`n3tx model`), interactive shell (`n3tx shell`), and a TUI dashboard. The gap is not capability -- it is *accessibility*. Today, a developer must know to run `python main.py` from the right directory, call `Product.list()` in a Python script, or hit `GET /Product` via curl. A CLI makes these operations discoverable and composable.

The honest weakness: N3TX currently requires ~8-10 minutes to go from `pip install` to a running application, compared to ~2-3 minutes for Django, Rails, and Laravel. This gap is entirely due to the absence of `n3tx new myapp` and `n3tx run`. The framework capabilities are competitive; the ceremony to access them is not.
...
| **Phase 4: TUI Dashboard** | `n3tx admin` (Textual-based) | ~20 hours | ~8 hrs/yr | Differentiator -- no other framework offers this |
...
- **Phase 1 (1 day):** Ship `n3tx run`, `n3tx models`, `n3tx describe <Model>`, and `n3tx migrate:status`. These are thin wrappers around existing functions. Use **Typer** as the CLI framework and **Rich** for terminal output. Add as an optional dependency: `pip install n3tx[cli]`. This gives developers a visible entry point and makes the schema inspectable from the terminal.

- **Phase 2 (1-2 days):** Add `n3tx list <table>`, `n3tx create <Model>`, `n3tx migrate`, `n3tx seed`. These wire `StorableMixin` CRUD and the migration system to terminal commands. The `create` command generates **schema-driven interactive prompts** -- validation constraints, required fields, widget hints -- all derived from the same schema that drives web forms. No other framework CLI can do this.

- **Phase 3 (1 day):** Add `n3tx scaffold <Model>`, `n3tx model <Name> field:type`, `n3tx shell`, `n3tx docs`. The shell uses **ptpython** with all registered models pre-imported. The model generator creates Python files from field specifications.

- **Phase 4 trigger condition:** Build the Textual TUI dashboard **only when** SSH-based server administration becomes a reported use case from actual users, OR when N3TX reaches 100+ external users and needs a differentiator. Until then, `matrix.html` serves as the data exploration layer. The CLI handles operations; the web frontend handles exploration.
...
- Do not use template-based scaffolding for code generation. N3TX's existing `scaffold.py` pattern -- generating from a live schema -- is architecturally correct and resistant to the drift that killed CRA. Maintain this discipline.
...
1. **Extract `N3TXApp.setup()`** (Week 1, ~2 hours). Refactor `build()` in `app.py` to separate model registration from HTTP server creation. This unblocks all CLI commands. Write a test that imports models, calls `setup()`, and verifies `registered_models` is populated without any HTTP server running.

2. **Ship Phase 1 CLI** (Week 1-2, ~8 hours). Create `n3tx/cli/` package with Typer app. Implement `run`, `models`, `describe`, `migrate:status`. Add `[project.scripts] n3tx = "n3tx.cli:app"` to `pyproject.toml`. Write CliRunner tests for each command. This is the minimum viable CLI.
...
4. **Run the 5-minute test** (Week 3). Have someone unfamiliar with N3TX install `n3tx[cli]` and try to create and run an application using only the CLI and `--help` output. Document every friction point. Fix the top 3.

5. **Update CLAUDE.md** (Week 4). Add CLI architecture, command reference, and testing instructions to the project guide. Update the "Testing Changes" section to include `n3tx run` as the primary server start command.

---

*Analysis based on 5 research documents comprising ~4,500 lines of technical research with 80+ cited sources, cross-referenced against N3TX v0.7.0 codebase architecture. All statistics reflect data available as of February 2026.*
