...
**PyBend** is a lightweight, declarative Python framework for building REST APIs with automatic CRUD operations, schema generation, and flexible storage backends. Define your data models once, and PyBend handles the routing, validation, database operations, and API documentation.
...
backend = FastAPIBackend(
    name="My API",
    version="1.0.0",
    description="API built with PyBend"
)
...
PyBend models inherit from `ProtoModel` and use class variables to configure behavior:
...
PyBend supports multiple storage backends:
...
PyBend can automatically generate join tables for many-to-many relationships:
...
PyBend automatically generates OpenAPI-compatible schemas for all models and endpoints.
...
PyBend includes a documentation generator:
...
PyBend includes automatic error handling with stack traces:
