...
import { NTTUser } from '../../components/ntt-user.js';
import { NTTItem } from '../../components/ntt-item.js';

describe('ntt-user.js (NTTUser)', () => {

  describe('custom element registration', () => {
    it('should be registered as ntt-user', () => {
      expect(customElements.get('ntt-user')).toBe(NTTUser);
    });
  });
  ...
  describe('styles getter', () => {
    it('should return a CSS URL string', () => {
      const el = document.createElement('ntt-user');
      const styles = el.styles;
      expect(typeof styles).toBe('string');
      expect(styles).toContain('ntt-user.css');
    });
  });
