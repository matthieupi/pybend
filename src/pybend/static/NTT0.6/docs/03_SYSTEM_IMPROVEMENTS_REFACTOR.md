# NTT Enhancement Plan: Review & Corrections
...
- Contradicts PyBend's "transparent, not magical" principle — the framework's most developer-facing config surface has zero guardrails
...
get #resolvedItemFactory() {
  ...
  // 3. Framework default
  return () => document.createElement('ntt-item');
}
```
...
```html
<script type="application/json" data-ntt-schema="Product">...</script>
<script type="application/json" data-ntt-data="Product">...</script>
```

In `NTT.ATTACH`, look up both by model name:

```js
const preloadedSchema = document.querySelector(`script[data-ntt-schema="${model}"]`);
const preloadedData = document.querySelector(`script[data-ntt-data="${model}"]`);
```

The NTT core already knows how to map model name → API endpoint (it's in the schema's `$id`). The HTML template shouldn't need to duplicate that knowledge.
...
Track A (Backend):                Track B (Frontend):
  ...
  §7 SSR pre-loading (backend)      §7 SSR pre-loading (NTT.js)
...
- §7 SSR pre-loading: backend template + NTT.js check (ship backend first, frontend second)
...
- Missing models (typo in `<ntt-list model="Prodcut">`)
...
| Schema fetch fails | `renderError(err)` | NTT.ATTACH error path |
...
```css
.ntt-loading, .ntt-error, .ntt-empty {
  padding: 1rem;
  text-align: center;
  color: var(--ntt-muted, #666);
}
.ntt-error { color: var(--ntt-error, #c00); }
```
...
- **File locations**: `components/ntt-element.js` is deleted, replaced by `components/NTTElement.js`
...
Existing pages using `<ntt-item>` and `<ntt-list>` will work (the custom element tags don't change), but any code that imports or extends the old classes will break.
...
```js
// components/ntt-element.js (OLD — becomes shim)
export { NTTElement } from './NTTElement.js';
console.warn('ntt-element.js is deprecated. Import from NTTElement.js instead.');
```
...
`<ntt-item>` and `<ntt-list>` tags NEVER change. They are the public API surface. Internal class names and file locations can change; tag names are permanent.

### What Must Be Documented

Before P0 ships, publish a migration checklist:

1. `import { NTTElement } from './ntt-element.js'` → `import { NTTElement } from './NTTElement.js'`
...
| **P2-B** | SSR pre-loading (NTT.js) | Frontend | Fixed naming convention |
