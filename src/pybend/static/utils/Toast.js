...
function getContainer() {
    ...
    container.id = 'ntt-toast-container';
...
export function showToast(message, type = 'error', duration = TOAST_DURATION) {
    ...
    toast.className = `ntt-toast ntt-toast--${type}`;
    toast.textContent = String(message);
    box.appendChild(toast);

    // Trigger entrance animation on next frame
    requestAnimationFrame(() => {
        toast.classList.add('ntt-toast--visible');
    });
...
function dismiss(toast) {
    ...
    toast.classList.remove('ntt-toast--visible');
    setTimeout(() => {
