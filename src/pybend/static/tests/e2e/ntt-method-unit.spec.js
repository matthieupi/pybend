/**
 * ntt-method — Comprehensive Unit Tests
 *
...
test.describe('ntt-method — Product Favorite Button', () => {
  ...
  test('favorite method element exists on product detail', async ({ page }) => {
    ...
    const hasFav = await page.locator('ntt-router').evaluate((r) => {
  ...
  test('favorite button has star icon SVG', async ({ page }) => {
    ...
    const hasStarSvg = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return false;
  ...
  test('favorite button is clickable (has .method-btn)', async ({ page }) => {
    ...
    const hasBtn = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return false;
      const method = item.shadowRoot.querySelector('ntt-method[method="favorite"]');
      if (!method?.shadowRoot) return false;
  ...
  test('favorite button uses button layout (pill-shaped)', async ({ page }) => {
    ...
    const style = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
  ...
  test('favorite button also present on sm list items', async ({ page }) => {
    ...
    const hasFavInList = await page.locator('ntt-list').evaluate((el) => {
      const items = el.shadowRoot?.querySelectorAll('ntt-item');
      if (!items?.length) return false;
      for (const item of items) {
        if (item.shadowRoot?.querySelector('ntt-method[method="favorite"]')) return true;
      }
      return false;
    });
...
test.describe('ntt-method — Product Comment Method (Inline)', () => {
  ...
  test('comment method element exists on product detail', async ({ page }) => {
    ...
    const hasComment = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return false;
  ...
  test('comment method has inline layout with textarea', async ({ page }) => {
    ...
    const inlineInfo = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return {};
      const method = item.shadowRoot.querySelector('ntt-method[method="comment"]');
      if (!method?.shadowRoot) return {};
      return {
  ...
  test('comment textarea has placeholder "Add your comment..."', async ({ page }) => {
    ...
    const placeholder = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return '';
      const method = item.shadowRoot.querySelector('ntt-method[method="comment"]');
      if (!method?.shadowRoot) return '';
  ...
  test('comment form has "Post" submit button', async ({ page }) => {
    ...
    const btnText = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return '';
      const method = item.shadowRoot.querySelector('ntt-method[method="comment"]');
      if (!method?.shadowRoot) return '';
...
test.describe('ntt-method — Comment Like Button (Nested)', () => {
  ...
  test('comment items have like method with heart icon', async ({ page }) => {
    ...
    const likeInfo = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return {};
      // Find a comment ntt-item inside the comments list-field
      const commentItems = item.shadowRoot.querySelectorAll('.list-field[data-value="comments"] ntt-item');
      for (const ci of commentItems) {
        const likeMethod = ci.shadowRoot?.querySelector('ntt-method[method="like"]');
...
test.describe('ntt-method — Schema Verification', () => {
...
test.describe('ntt-method — Edge Cases', () => {
