...
test.describe('accessibility — Keyboard Navigation', () => {
  ...
  test('Tab order includes topbar elements first', async ({ page }) => {
    ...
    const isTopbar = await page.evaluate(() => {
      const el = document.activeElement;
      return el?.tagName?.toLowerCase() === 'ntt-topbar' ||
             el?.closest?.('ntt-topbar') !== null;
    });
    // First focus might be on ntt-topbar or a link inside it
...
test.describe('accessibility — ARIA & Semantics', () => {
  ...
  test('buttons have accessible text content or aria-label', async ({ page }) => {
    ...
    const topbarButtons = await page.locator('ntt-topbar').evaluate((el) => {
  ...
  test('navigation links have descriptive text', async ({ page }) => {
    ...
    const links = await page.locator('ntt-topbar').evaluate((el) => {
  ...
  test('form inputs in edit mode have id attributes for label association', async ({ page }) => {
    ...
    const inputs = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
...
test.describe('accessibility — Focus Management', () => {
  ...
  test('after opening edit mode, name input receives focus', async ({ page }) => {
    ...
    const hasFocusedInput = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return false;
...
test.describe('accessibility — Screen Reader Support', () => {
  ...
  test('navigation is present via ntt-topbar', async ({ page }) => {
    await page.goto(APP_URL);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1000);

    const hasNav = await page.locator('ntt-topbar').evaluate((el) => {
  ...
  test('list has a heading element identifying the content', async ({ page }) => {
    ...
    const headingText = await page.locator('ntt-list').evaluate((el) => {
...
test.describe('accessibility — Edge Cases', () => {
  ...
  test('images have alt attributes', async ({ page }) => {
    ...
    const images = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return [];
  ...
  test('buttons have title or text for screen readers', async ({ page }) => {
    ...
    const buttons = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      if (!item?.shadowRoot) return [];
