 ...
 *
 * Sets PYBEND_SQLITE_DB to a temp file so the server launched by
...
export default async function globalSetup() {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'pybend-e2e-'));
  ...
  execSync(
    `cd ${EXAMPLE_DIR} && PYBEND_SQLITE_DB="${dbPath}" python3 seed.py`,
