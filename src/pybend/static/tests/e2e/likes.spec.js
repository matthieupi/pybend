...
test.describe('Likes', () => {
  ...
  test('like button visible on product list items', async ({ page }) => {
    ...
    const hasLikeMethod = await page.locator('ntt-list').evaluate((list) => {
      const items = list.shadowRoot?.querySelectorAll('ntt-item');
      if (!items?.length) return false;
      const first = items[0];
      return !!first?.shadowRoot?.querySelector('ntt-method[method="like"]');
    });
  ...
  test('like button shows count badge', async ({ page }) => {
    ...
    const likeInfo = await page.locator('ntt-router').evaluate((r) => {
      ...
      const method = item.shadowRoot.querySelector('ntt-method[method="like"]');
