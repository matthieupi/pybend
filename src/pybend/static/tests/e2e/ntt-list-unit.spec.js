/**
 * ntt-list — Comprehensive Unit Tests
 *
...
test.describe('ntt-list — Basic Rendering', () => {
  ...
  test('ntt-list element is visible on page load', async ({ page }) => {
  ...
  test('list-header h1 says "Products"', async ({ page }) => {
    ...
    const header = await page.locator('ntt-list').evaluate((el) => {
  ...
  test('list-count shows total product count', async ({ page }) => {
    ...
    const count = await page.locator('ntt-list').evaluate((el) => {
  ...
  test('list-grid contains at least 3 ntt-item elements', async ({ page }) => {
    await page.goto(APP_URL);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
    ...
    const itemCount = await page.locator('ntt-list').evaluate((el) => {
      ...
      return grid ? grid.querySelectorAll('ntt-item').length : 0;
    });
    expect(itemCount).toBeGreaterThanOrEqual(3);
  });

  test('each ntt-item has a display attribute', async ({ page }) => {
    await page.goto(APP_URL);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);

    const displays = await page.locator('ntt-list').evaluate((el) => {
...
test.describe('ntt-list — Item Display Content', () => {
  ...
  test('first item shows product name', async ({ page }) => {
    ...
    const name = await page.locator('ntt-list').evaluate((el) => {
  ...
  test('each item has a .card element with data-display', async ({ page }) => {
    ...
    const cardDisplays = await page.locator('ntt-list').evaluate((el) => {
      const items = el.shadowRoot?.querySelectorAll('ntt-item');
      if (!items?.length) return [];
  ...
  test('all product names are non-empty', async ({ page }) => {
    ...
    const names = await page.locator('ntt-list').evaluate((el) => {
      const items = el.shadowRoot?.querySelectorAll('ntt-item');
      if (!items?.length) return [];
      return Array.from(items).map(item => {
        const nameEl = item.shadowRoot?.querySelector('[data-value="name"]') ||
...
test.describe('ntt-list — Click-to-Navigate', () => {
  ...
  test('after click, ntt-router shows the detail view', async ({ page }) => {
    await page.goto(APP_URL);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
...
test.describe('ntt-list — Structure', () => {
  ...
  test('list has list-header and list-grid in shadow DOM', async ({ page }) => {
    ...
    const structure = await page.locator('ntt-list').evaluate((el) => {
  ...
  test('list has model attribute set to "Product"', async ({ page }) => {
    ...
    const model = await page.locator('ntt-list').evaluate((el) => {
  ...
  test('items have select-target attribute pointing to list', async ({ page }) => {
    ...
    const selectTargets = await page.locator('ntt-list').evaluate((el) => {
      const items = el.shadowRoot?.querySelectorAll('ntt-item');
      if (!items?.length) return [];
  ...
  test('items have data-value attribute with ref URL', async ({ page }) => {
    ...
    const refs = await page.locator('ntt-list').evaluate((el) => {
      const items = el.shadowRoot?.querySelectorAll('ntt-item');
      if (!items?.length) return [];
  ...
  test('count badge matches actual number of rendered items', async ({ page }) => {
    ...
    const { countText, renderedCount } = await page.locator('ntt-list').evaluate((el) => {
      const sr = el.shadowRoot;
      const countText = sr?.querySelector('.list-count')?.textContent || '';
      const items = sr?.querySelectorAll('.list-grid ntt-item');
...
test.describe('ntt-list — Responsive Breakpoints', () => {
  ...
  for (const bp of breakpoints) {
    test(`list renders correctly at ${bp.name}`, async ({ page }) => {
      ...
      const visible = await page.locator('ntt-list').evaluate((el) => {
        const sr = el.shadowRoot;
        return {
          hasGrid: !!sr?.querySelector('.list-grid'),
          itemCount: sr?.querySelectorAll('.list-grid ntt-item').length || 0,
        };
      });
