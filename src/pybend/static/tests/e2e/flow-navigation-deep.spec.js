...
test.describe('Navigation — List to Detail to Back', () => {
  ...
  test('clicking different products shows different detail content', async ({ page }) => {
    ...
    const content1 = await page.locator('ntt-router').evaluate((r) => {
    ...
    const content2 = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      return item?.shadowRoot?.querySelector('.card')?.textContent || '';
    });
...
test.describe('Navigation — Hash Changes', () => {
  ...
  test('hash change without reload updates router content', async ({ page }) => {
    ...
    const hasRouterContent = await page.locator('ntt-router').evaluate((r) => {
      return !!r.shadowRoot?.querySelector('.router-content');
    });
    ...
    const hasList = await page.evaluate(() => {
      return !!document.querySelector('ntt-list');
    });
...
test.describe('Navigation — Auth State Changes', () => {
  ...
  test('edit button appears after login on product detail', async ({ page }) => {
    ...
    const hasEditAnon = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
    ...
    const hasEditAuth = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      return !!item?.shadowRoot?.querySelector('.edit-btn');
    });
  ...
  test('edit button disappears after logout', async ({ page }) => {
    ...
    const hasEditBefore = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      return !!item?.shadowRoot?.querySelector('.edit-btn');
    });
    ...
    const hasEditAfter = await page.locator('ntt-router').evaluate((r) => {
      const item = r.shadowRoot?.querySelector('ntt-item');
      return !!item?.shadowRoot?.querySelector('.edit-btn');
    });
...
test.describe('Navigation — State Consistency', () => {
  ...
  test('URL hash always matches displayed content', async ({ page }) => {
    ...
    const hasListAtRoot = await page.evaluate(() => !!document.querySelector('ntt-list'));
    ...
    const hasItemAtDetail = await page.locator('ntt-router').evaluate((r) => {
      return !!r.shadowRoot?.querySelector('ntt-item');
    });
    ...
    const hasListAfterBack = await page.evaluate(() => !!document.querySelector('ntt-list'));
  ...
  test('router shows back button on detail view, hidden at root', async ({ page }) => {
    ...
    const hasBackAtRoot = await page.locator('ntt-router').evaluate((r) => {
    ...
    const hasBackAtDetail = await page.locator('ntt-router').evaluate((r) => {
      return !!r.shadowRoot?.querySelector('.back-btn');
    });
