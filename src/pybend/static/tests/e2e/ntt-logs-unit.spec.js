/**
 * ntt-logs — Comprehensive Unit Tests
 *
...
test.describe('ntt-logs — Toggle Behavior', () => {
  ...
  test('toggle button visible and panel starts closed', async ({ page }) => {
    ...
    const state = await page.locator('ntt-logs').evaluate((el) => {
  ...
  test('panel slides in from right (transform transition)', async ({ page }) => {
    ...
    const transform = await page.locator('ntt-logs').evaluate((el) => {
...
test.describe('ntt-logs — Log Entries', () => {
  ...
  test('entries exist after page load', async ({ page }) => {
    ...
    const count = await page.locator('ntt-logs').evaluate((el) => {
  ...
  test('each entry has level-dot, level, time elements', async ({ page }) => {
    ...
    const entryStructure = await page.locator('ntt-logs').evaluate((el) => {
  ...
  test('entry level classes are valid log levels', async ({ page }) => {
    ...
    const levels = await page.locator('ntt-logs').evaluate((el) => {
      const entries = el.shadowRoot?.querySelectorAll('.log-list .entry');
  ...
  test('entries have data-level attribute for filtering', async ({ page }) => {
    ...
    const hasDataLevel = await page.locator('ntt-logs').evaluate((el) => {
      const entries = el.shadowRoot?.querySelectorAll('.log-list .entry');
  ...
  test('entry time shows valid HH:MM:SS format', async ({ page }) => {
    ...
    const time = await page.locator('ntt-logs').evaluate((el) => {
...
test.describe('ntt-logs — Filtering', () => {
...
test.describe('ntt-logs — Clear', () => {
  ...
  test('clear button removes all entries and resets badge', async ({ page }) => {
    ...
    const afterState = await page.locator('ntt-logs').evaluate((el) => {
      return {
  ...
  test('new logs still appear after clear', async ({ page }) => {
    ...
    const newCount = await page.locator('ntt-logs').evaluate((el) => {
      return el.shadowRoot?.querySelectorAll('.log-list .entry').length || 0;
    });
...
test.describe('ntt-logs — JSON Expansion', () => {
  ...
  test('JSON nodes exist and start collapsed (depth > 0)', async ({ page }) => {
    ...
    const jsonInfo = await page.locator('ntt-logs').evaluate((el) => {
...
test.describe('ntt-logs — Badge Counter', () => {
  ...
  test('badge shows non-zero count after page load', async ({ page }) => {
    ...
    const badge = await page.locator('ntt-logs').evaluate((el) => {
  ...
  test('badge count reflects Logging.size (capped at buffer max)', async ({ page }) => {
    ...
    const { badge, entryCount } = await page.locator('ntt-logs').evaluate((el) => {
...
test.describe('ntt-logs — Edge Cases', () => {
  ...
  test('log panel is fixed position at bottom-right', async ({ page }) => {
    ...
    const position = await page.locator('ntt-logs').evaluate((el) => {
  ...
  test('entries with special characters render safely (no XSS)', async ({ page }) => {
    ...
    const hasScriptTag = await page.locator('ntt-logs').evaluate((el) => {
  ...
  test('toggle button is a round circle (border-radius: 50%)', async ({ page }) => {
    ...
    const radius = await page.locator('ntt-logs').evaluate((el) => {
