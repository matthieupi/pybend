...
describe('Toast.js', () => {
  ...
  beforeEach(() => {
    ...
    const existing = document.getElementById('ntt-toast-container');
  ...
  describe('showToast(message, type, duration)', () => {
    ...
    it('should create a toast element in the DOM', () => {
      ...
      const container = document.getElementById('ntt-toast-container');
    ...
    it('should apply error class by default', () => {
      ...
      expect(toast.classList.contains('ntt-toast--error')).toBe(true);
    });

    it('should apply success class when type is success', () => {
      const toast = showToast('Saved', 'success');
      expect(toast.classList.contains('ntt-toast--success')).toBe(true);
    });

    it('should apply info class when type is info', () => {
      const toast = showToast('Loading', 'info');
      expect(toast.classList.contains('ntt-toast--info')).toBe(true);
    });
    ...
    it('should add the ntt-toast--visible class after creation via rAF', async () => {
      ...
      expect(toast.classList.contains('ntt-toast--visible')).toBe(true);
    });
  });
  ...
  describe('style injection', () => {
    ...
    it('should inject a <style> element into <head>', () => {
      ...
      const hasToastStyle = Array.from(styles).some(s =>
        s.textContent.includes('.ntt-toast')
      );
  ...
  describe('container', () => {
    ...
    it('should reuse existing container', () => {
      showToast('One');
      showToast('Two');
      const containers = document.querySelectorAll('#ntt-toast-container');
