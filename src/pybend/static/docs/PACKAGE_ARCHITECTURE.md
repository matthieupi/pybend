# NTT Package Architecture

Module boundaries, dependency analysis, and packaging strategy for the NTT frontend framework and its PyBend backend.
...
├─────────────────────────────────────────────┤
│  Core: NTT, TT, DynamicClass,              │  Schema-driven data layer
...
static/
  ...
  core/
    ...
    NTT.js                      TT + NTT + DynamicClass factory (prototype())
  ...
  components/
    ntt-element.js              Entity-aware base component (to be merged with Component)
    ntt-item.js                 Single entity display/edit (<ntt-item>)
    ntt-list.js                 Collection display (<ntt-list>)
    ntt-method.js               Method invocation buttons (<ntt-method>)
...
NTT.js ← Actor, Matrix, TX, Observable, NetworkAdapter, Assert, Utils, Logging, config
  └── prototype() creates DynamicClass (extends NTT, applies Actor.subclass + Observable)

Component.js ← Actor, Matrix, TX, Utils, Logging, [ntt-item.js ← CIRCULAR]

ntt-element.js ← Component, NTT, TX, Matrix, Utils, Logging, [ntt-item.js ← CIRCULAR]

ntt-item.js ← ntt-element, NTT, ntt-method, Formidable, TX, Assert, Logging
ntt-list.js ← ntt-element, ntt-item, Observable, Actor, Utils, Logging
ntt-method.js ← NTT

form.js ← (no explicit imports, uses global NTT — missing import bug)
```

### Circular Dependencies

1. **Component.js ↔ ntt-item.js**: Component.js has `import '../components/ntt-item.js'` (side-effect import for `customElements.define`). ntt-item.js imports Component.js through ntt-element.js. Managed by JS module system but architecturally wrong — a base class importing a grandchild. **Resolved by the component refactor** (see UI_SEPARATION_CONCERNS.md).

2. **form.js → NTT (implicit global)**: Formidable calls `NTT.get()` without importing NTT. Works because `window.NTT = NTT` is set in NTT.js, but fragile. Needs an explicit import.
...
**Files:**
```
NTT.js (TT, NTT, prototype()/DynamicClass factory)
...
- NTT: global schema registry, ATTACH/SCHEMA protocol, DynamicClass lifecycle
...
- React/Vue/Svelte developers with a PyBend backend who want schema-driven data without NTT's web components
- Anyone wanting automatic entity management from JSON Schema

**Example standalone usage:**
```js
import { NTT } from '@ntt/core';
import { matrix } from '@ntt/actor';

// React component using NTT data layer
function ProductList() {
  const [products, setProducts] = useState([]);

  useEffect(() => {
    NTT.attach('Product', (DynamicClass) => {
...
**What it provides:**
- Component: the bridge between HTMLElement and the NTT data layer
  - Shadow DOM setup with stylesheet hook (`get styles()`)
  - Actor identity (addr, hash, Matrix registration)
  - Schema resolution: `model` attribute → ATTACH TX → NTT core → `define(proto)` → `definedCallback()`
...
**Who would use this:**
- Developers building custom web components for their domain with PyBend
...
**Independent value: HIGH within the PyBend ecosystem.** This is the layer most developers interact with.
...
**Files:**
```
ntt-item.js      — Zero-config entity component (auto-renders via Formidable)
ntt-list.js      — Zero-config collection component
ntt-method.js    — Method invocation buttons
Formidable.js    — Schema-to-HTML form generator (renamed from form.js)
```

**What it provides:**
- NTTItem: extends NTTElement with Formidable-powered auto-rendering, edit/save toggle, input binding, method button rendering. Registered as `<ntt-item>`.
- NTTList: extends ListElement with zero configuration. Registered as `<ntt-list>`.
- NTTMethod: renders callable methods from schema as buttons with parameter forms. Registered as `<ntt-method>`.
...
- Getting started with PyBend (zero frontend code needed)
...
- Patches NTT.ATTACH to check for inline `<script type="application/json" data-ntt-schema="...">` before making network requests
...
- **Dependency:** `@ntt/core` (patches NTT behavior)
...
| `@ntt/core` | Schema-driven data layer, rendering-framework-agnostic | React/Vue/Svelte developers using PyBend backend |
| `@ntt/components` | DOM-specific base classes developers extend | PyBend web component developers |
| `@ntt/kit` | Opinionated defaults that shouldn't be in the base | Rapid prototyping, admin panels |
| `@ntt/preload` | Deploy-time optimization, not universally needed | Server-rendered PyBend apps |
...
| Transport + `@ntt/core` | NetworkAdapter only speaks NTT's TX protocol. Useless without NTT. |
| Formidable + `@ntt/kit` | Formidable only exists to serve NTTItem's auto-render. No standalone consumer. |
| Backend UI schema extensions + PyBend | ~30-40 lines in ProtoModel.schema(). Not worth a package boundary. |
...
All backend enhancements belong in the existing `pybend` package:
...
- Best as a CLI subcommand: `python -m pybend scaffold Product`
...
1. `Component.js` imports `ntt-item.js` (tier 3 importing tier 4) — dies in refactor
2. `form.js` uses global `window.NTT` without import — needs explicit import from `@ntt/core`
3. `ntt-list.js` calls `Actor.subclass(List)` directly (tier 4 reaching into tier 1 internals) — unnecessary, inherited through Component
...
ntt/
  ...
  core/                     ← @ntt/core boundary
    index.js                  Barrel: exports NTT, TT
    NTT.js
    transport/
      index.js
      HTTP.js
  ...
  kit/                      ← @ntt/kit boundary
    ...
    ntt-item.js               Zero-config entity component
    ntt-list.js               Zero-config list component
    ntt-method.js             Method buttons
...
// kit/index.js
export { NTTItem } from './ntt-item.js';
export { NTTList } from './ntt-list.js';
export { NTTMethod } from './ntt-method.js';
...
<script type="module">
  import './kit/index.js';  // Registers <ntt-item>, <ntt-list>, <ntt-method>
</script>
<ntt-list model="Product"></ntt-list>
```
...
|---|---|---|
| UI hints in schema | Backend (pybend) | `__ui__` class variable and `json_schema_extra` ui keys on fields. Schema emits `ui` section with groups, field_order, widget types, display modes. |
| Slot-based list templates | `@ntt/components` (ListElement) | `<template item-template>` in light DOM provides child element template. Resolution chain: template > `item-tag` attr > `childTag` property > schema hint > `'ntt-item'` fallback. |
...
| Schema pre-loading | `@ntt/preload` addon | NTT.ATTACH checks for inline `<script data-ntt-schema>` before network fetch. Eliminates async bootstrap latency. |
...
| Relationship rendering | `@ntt/kit` (Formidable) | ListRef fields auto-render as nested `<ntt-list>` scoped to parent. |
...
1. **The Actor system has genuine independent utility** beyond NTT and PyBend
...
4. **NTT.js currently imports Actor, Matrix, TX, Observable directly** — moving these to `actor/` makes the dependency explicit via an import path change
