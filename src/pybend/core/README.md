...
| `methods[m].ui` | `<ntt-method>` | Button layout, icon, count-field for social actions |
...
This creates `POST /products/{id}/like`, appears in `schema.methods`, and the frontend renders it as a clickable `<ntt-method>` button -- all from one decorator. Button-layout methods (like, favorite) render as compact icon + count pills via `__ui__.methods` hints.
