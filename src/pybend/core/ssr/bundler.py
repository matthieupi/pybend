"""
JavaScript bundler for PyBend SSR.
...
def _rewrite_import_meta_urls(work_dir):
    ...
    (e.g. ``http://host/components/ntt-list.js``).  In IIFE mode this
    is unavailable, so we rewrite each occurrence to an expression that
    reconstructs the correct URL at runtime::

        import.meta.url  →  (window.location.origin+'/components/ntt-list.js')

    This keeps ``new URL('./ntt-list.css', import.meta.url)`` resolving
...
def _merge_static_dirs(static_dirs):
    ...
    """
    work_dir = tempfile.mkdtemp(prefix='pybend_ssr_')
...
def _inline_css_imports(work_dir):
    ...
    for js_file in work_path.rglob('*.js'):
        ...
        def _replace_css(match, _js_file=js_file):
            css_rel = match.group(1)  # e.g. './ntt-list.css'
...
def discover_css_deps(static_dirs):
    ...
    Returns:
        ...
        (e.g. ``['./components/ntt-item.css', './components/ntt-list.css']``).
    """
    css_paths = set()
    for sd in static_dirs:
        sd_path = Path(sd)
