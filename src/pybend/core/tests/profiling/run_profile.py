...
Usage:
    ...
    python -m pybend.core.tests.profiling.run_profile baseline
    python -m pybend.core.tests.profiling.run_profile optimized
"""
import json
import os
import subprocess
...
def start_server(db_path, label='run'):
    """Start the PyBend server on an isolated port with a dedicated DB."""
    env = os.environ.copy()
    env['PYBEND_PROFILING'] = '1'
    env['PYBEND_SQLITE_DB'] = str(db_path)
    env['PYBEND_PROFILING_DIR'] = str(PROFILING_DIR)
    env['PYBEND_PORT'] = str(PROFILING_PORT)
    env['PYBEND_PROFILING_LABEL'] = label
...
def main():
    if len(sys.argv) < 2:
        print('Usage: python -m pybend.core.tests.profiling.run_profile <label>')
