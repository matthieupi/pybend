...
from pybend.core.app import create_app, _resolve_ssr
from pybend.core.storage.sqlite_storage import SQLiteStorage
from pybend.core import config
from pybend.core.authorize import configure as auth_configure
from pybend.core.utils.registrar import registered_models
from pybend.core.ssr.bundler import (
    parse_html_modules,
    build_bundle,
    discover_css_deps,
    _find_esbuild_bin,
...
from pybend.core.ssr.html import (
    build_schema_tags,
    inject_schemas,
    inject_bundle,
    inject_full,
...
def ssr_app(_isolated_models, static_dir, db_path):
    """Create a PyBend app with SSR schema mode (backward compat: ssr=True)."""
...
def no_ssr_app(_isolated_models, static_dir, db_path):
    """Create a PyBend app with SSR disabled (default)."""
    auth_configure(jwt_secret=config.JWT_SECRET, jwt_expiry_hours=1)
...
def _extract_schema_tags(html: str) -> list[tuple[str, dict]]:
    ...
    pattern = r'<script type="application/json" data-ntt-schema="([^"]+)">(.+?)</script>'
    results = []
...
class TestSSRBundleMode:
    ...
    def bundle_app(self, _isolated_models, static_dir_with_modules):
        auth_configure(jwt_secret=config.JWT_SECRET, jwt_expiry_hours=1)
        with patch('pybend.core.ssr.bundler.build_bundle', return_value=MOCK_BUNDLE_JS):
            return create_app(
                models=[SSRProduct],
...
class TestHTMLTransforms:
    ...
    def test_inject_schemas(self):
        ...
        assert 'data-ntt-schema="SSRProduct"' in result
    ...
    def test_inject_full(self):
        ...
        assert 'modulepreload' not in result
        assert 'type="module"' not in result
        assert 'data-ntt-schema' in result
        assert '<script>var x = 1;</script>' in result

    def test_build_schema_tags(self):
        tags = build_schema_tags({'SSRProduct': SSRProduct})
        assert 'data-ntt-schema="SSRProduct"' in tags
        # Verify it's valid JSON inside
        match = re.search(r'data-ntt-schema="SSRProduct">(.+?)</script>', tags)
        assert match
    ...
    def test_inject_css_preloads(self):
        ...
        result = inject_css_preloads(html, [
            './components/ntt-item.css',
            './components/ntt-list.css',
        ])
        assert '<!-- SSR: CSS preloads -->' in result
        assert '<link rel="preload" href="./components/ntt-item.css" as="style" crossorigin>' in result
        assert '<link rel="preload" href="./components/ntt-list.css" as="style" crossorigin>' in result
    ...
    def test_inject_bundle_strips_css_preloads(self):
        ...
        html = textwrap.dedent("""\
            <html>
            <head>
                <link rel="preload" href="./components/ntt-item.css" as="style" crossorigin>
                <link rel="preload" href="./components/ntt-list.css" as="style" crossorigin>
                <link rel="modulepreload" href="./config.js">
                <script type="module" src="./theme.js"></script>
            </head>
            <body>
...
class TestBundlerHelpers:
    ...
    def test_rewrite_import_meta_urls(self, tmp_path):
        ...
        d.mkdir()
        (d / 'components').mkdir()
        (d / 'components' / 'ntt-list.js').write_text(
        ...
        rewritten = (d / 'components' / 'ntt-list.js').read_text()
        assert 'import.meta.url' not in rewritten
        assert "window.location.origin+'/components/ntt-list.js'" in rewritten
    ...
    def test_inline_css_imports(self, tmp_path):
        ...
        d.mkdir()
        (d / 'components').mkdir()
        css_content = ".ntt-list { display: block; }"
        (d / 'components' / 'ntt-list.css').write_text(css_content)
        (d / 'components' / 'ntt-list.js').write_text(
            "get styles() { return new URL('./ntt-list.css', import.meta.url).href; }"
        )
    ...
    def test_inline_css_const_pattern(self, tmp_path):
        ...
        (d / 'ntt-topbar.css').write_text(css_content)
        (d / 'ntt-topbar.js').write_text(
            "const TOPBAR_CSS = new URL('./ntt-topbar.css', import.meta.url).href;"
        )

        _inline_css_imports(str(d))

        rewritten = (d / 'ntt-topbar.js').read_text()
        b64 = base64.b64encode(css_content.encode()).decode('ascii')
    ...
    def test_discover_css_deps(self, tmp_path):
        ...
        (d / 'components' / 'ntt-list.css').write_text('.ntt-list {}')
        (d / 'components' / 'ntt-item.js').write_text(
            "get styles() { return new URL('./ntt-item.css', import.meta.url).href; }"
        )
        (d / 'components' / 'ntt-item.css').write_text('.ntt-item {}')
        (d / 'config.js').write_text("export const config = {};")  # no CSS dep

        deps = discover_css_deps([str(d)])
        assert deps == ['./components/ntt-item.css', './components/ntt-list.css']
    ...
    def test_discover_css_deps_multiple_dirs(self, tmp_path):
        ...
        fw.mkdir()
        (fw / 'components').mkdir()
        (fw / 'components' / 'ntt-list.js').write_text(
            "get styles() { return new URL('./ntt-list.css', import.meta.url).href; }"
        )
        (fw / 'components' / 'ntt-list.css').write_text('.ntt-list {}')

        app = tmp_path / 'app'
        app.mkdir()
        (app / 'components').mkdir()
        (app / 'components' / 'ntt-user.js').write_text(
            "get styles() { return new URL('./ntt-user.css', import.meta.url).href; }"
        )
        (app / 'components' / 'ntt-user.css').write_text('.ntt-user {}')

        deps = discover_css_deps([str(app), str(fw)])
        assert './components/ntt-list.css' in deps
        assert './components/ntt-user.css' in deps
