...
```python
from pybend import AgentActor, create_app
...
PyBend does not build an LLM client, agent loop, or context manager.
...
PyBend's job is purely the **binding layer**: discover tools from actors,
...
| `pybend/__init__.py` | Exports `AgentMixin`, `AgentActor` |
