...
def run_tests():
    ...
    with sync_playwright() as p:
        ...
        page_state = page.evaluate("""() => ({
            router: !!document.querySelector('ntt-router'),
            topbar: !!document.querySelector('ntt-topbar'),
        })""")
        check("ntt-router present", page_state.get('router'))
        check("ntt-topbar present", page_state.get('topbar'))

        # 8. Topbar Favorites link
        has_fav_link = page.evaluate("""() => {
            const tb = document.querySelector('ntt-topbar');
            return !!tb?.shadowRoot?.querySelector('a[href="#@favorites"]');
        }""")
        check("Favorites link in topbar", has_fav_link)

        # 9. ntt-list rendered with products
        # The ntt-router contains an ntt-list, but it might be in shadow DOM or slotted
        list_info = page.evaluate("""() => {
            // Try direct child first
            let list = document.querySelector('ntt-list');
            // If not found, check inside ntt-router shadow
            if (!list) {
                const router = document.querySelector('ntt-router');
                list = router?.shadowRoot?.querySelector('ntt-list');
            }
            if (!list) return { found: false };
            const sr = list.shadowRoot;
            if (!sr) return { found: true, shadow: false };
            const items = sr.querySelectorAll('ntt-item');
            return { found: true, shadow: true, itemCount: items.length };
        }""")
        check("ntt-list found", list_info.get('found'), f"state={list_info}")
        ...
        star_info = page.evaluate("""() => {
            // Find first product ntt-item
            const list = document.querySelector('ntt-list');
            if (!list?.shadowRoot) return { found: false, reason: 'no list' };
            const item = list.shadowRoot.querySelector('ntt-item');
            if (!item?.shadowRoot) return { found: false, reason: 'no item' };
            // Look for ntt-method with icon=star
            const method = item.shadowRoot.querySelector('ntt-method[icon="star"]');
            if (!method) {
                // Check all ntt-methods
                const all = Array.from(item.shadowRoot.querySelectorAll('ntt-method'));
                return { found: false, reason: `${all.length} ntt-methods, none with icon=star`,
        ...
        heart_info = page.evaluate("""() => {
            ...
            // Look for nested ntt-item (comments)
            const commentItems = item.shadowRoot.querySelectorAll('ntt-item');
            ...
            const method = comment.shadowRoot.querySelector('ntt-method[icon="heart"]');
        ...
        reply_info = page.evaluate("""() => {
            ...
            const replyMethod = comment.shadowRoot.querySelector('ntt-method[method="reply"]');
        ...
        indent_info = page.evaluate("""() => {
            ...
            for (const item of items) {
                ...
                const nested = item.shadowRoot.querySelectorAll('ntt-item');
