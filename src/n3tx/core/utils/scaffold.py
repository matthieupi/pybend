...
# Where generated components go (relative to n3tx root)
...
def _field_html(key: str, field_def: dict, model_name: str, mode: str = 'display') -> str:
    ...
    if t == 'array' and field_def.get('items'):
        ...
        return (
            ...
            f'          ${{({key} ?? []).map(c =>\n'
            f'            `<ntx-item ref="${{c}}" data-model="{ref_model}"></ntx-item>`\n'
            f'          ).join(\'\')}}\n'
            f'        </div>'
        )
...
def scaffold_item(schema: dict) -> str:
    ...
    for mname, mdef in methods.items():
        ...
        method_lines.append(
            f'        <ntx-method model="{model_name}" '
            f'uuid="${{this.value?.id || \'\'}}" '
            f'method="{mname}" label="{label}"></ntx-method>'
        )
