...
NTT.SCHEMA = function(data, tx) {
    performance.mark('ntx-schema-start');
    const result = _origSCHEMA(data, tx);
    performance.mark('ntx-schema-end');
    performance.measure('NTT.SCHEMA:' + (data?.__name__ || 'unknown'), 'ntx-schema-start', 'ntx-schema-end');
    return result;
};
...
window.__NTT_PERF_HARNESS__ = true;
