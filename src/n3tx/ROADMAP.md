...
Full-stack pagination implemented: `sqlite_storage.py` COUNT + LIMIT/OFFSET, `storable_mixin.py` passthrough, `routes_fastapi.py` query params, `NetworkAdapter.js` query param encoding, `N3TX.js` `{data, meta}` detection, `ListElement` "Load More" button with count/total display.

---

### ~~3. DELETE Action in Frontend UI~~ DONE (v0.7.0)

Delete button in `ntx-item` md/lg/xl sizes with confirmation dialog and ABAC check. Routes through DynamicClass for proper registry cleanup. Uses `ref` URL for nested entities. Delete also works in sm size with hover-reveal action buttons.
...
**Problem:** `ntx-item.js` uses hardcoded heuristics for field selection per size. `xs()` shows name only, `sm()` picks first 3 fields via `#topFields(3)`, `md()` shows full form, `lg()`/`xl()` just delegate to `md()`. No schema control.
...
- `ntx-item.js`: Replace `#topFields()` heuristic with `schema.ui.display_modes[size]` lookup, fall back to current heuristic if absent
...
| List UI | `ntx-list.js` | Add search bar in list header |
...
**Current state:** Tests exist at `src/n3tx/core/tests/` — `test_models.py`, `test_storage.py`, `test_api.py` with `conftest.py` (temp DB isolation via `monkeypatch`). Coverage is thin.
...
**Problem:** `index.html` hardcodes `<ntx-list model="Product">`. No endpoint to discover models. No selector UI.
...
- **Navigation:** Click model -> Router NAVIGATE -> ntx-router mounts `<ntx-list model="ModelName">`
- Multiple `<ntx-list>` instances can coexist safely (independent Matrix registration)

**Files:** `routes_fastapi.py` (1 new route), `index.html` (layout refactor), `ntx-topbar.js` or new `ntx-model-selector.js`
...
- `N3TX.js`: 8 calls (mixed debug/error)
