...
### Widget Extension (JS)
On the frontend, `form.js` and `ntt-item.js` dispatch to registered JS Widget instances (`getWidgetForField()`) before falling through to type-based rendering. Fields without `ui.widget` render identically to before (zero-risk).
