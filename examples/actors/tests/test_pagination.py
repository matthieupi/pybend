...
class TestPaginationStructure:
    ...
    def test_meta_limit_matches_request(self, client, alice_token, seed_data):
        resp = client.get("/Product/_?limit=3&offset=0", headers=auth_header(alice_token))
        meta = resp.json()["meta"]
        assert meta["limit"] == 3

    def test_meta_offset_matches_request(self, client, alice_token, seed_data):
        resp = client.get("/Product/_?limit=2&offset=2", headers=auth_header(alice_token))
        meta = resp.json()["meta"]
...
class TestTotalCount:
    ...
    def test_total_count_reflects_all_records(self, client, alice_token, seed_data):
        resp = client.get("/Product/_?limit=1&offset=0", headers=auth_header(alice_token))
        meta = resp.json()["meta"]
    ...
    def test_total_count_consistent_across_pages(self, client, alice_token, seed_data):
        resp1 = client.get("/Product/_?limit=2&offset=0", headers=auth_header(alice_token))
        resp2 = client.get("/Product/_?limit=2&offset=2", headers=auth_header(alice_token))
...
class TestHasMoreFlag:
    ...
    def test_has_more_false_on_last_page(self, client, alice_token, seed_data):
        # Get total first
        resp_total = client.get("/Product/_?limit=1&offset=0", headers=auth_header(alice_token))
        total = resp_total.json()["meta"]["total"]

        # Fetch last page
        resp = client.get(f"/Product/_?limit=2&offset={max(total - 2, 0)}",
...
class TestBoundaryConditions:
    ...
    def test_offset_beyond_total_returns_empty(self, client, alice_token, seed_data):
        resp = client.get("/Product/_?limit=10&offset=10000", headers=auth_header(alice_token))
        data = resp.json()
        assert data["data"] == []
    ...
    def test_limit_without_offset_defaults_offset_to_zero(self, client, alice_token, seed_data):
        resp = client.get("/Product/_?limit=2", headers=auth_header(alice_token))
        meta = resp.json()["meta"]
...
class TestPaginationDefaults:
    ...
    def test_default_limit_returns_all(self, client, alice_token, seed_data):
        """Explicit collection route without limit returns a plain list."""
    ...
    def test_limit_one_returns_single_item(self, client, alice_token, seed_data):
        ...
        resp = client.get("/Product/_?limit=1", headers=auth_header(alice_token))
        data = resp.json()
    ...
    def test_exact_boundary_page(self, client, alice_token, seed_data):
        ...
        if total > 1:
            ...
            boundary_resp = client.get(
                f"/Product/_?limit=1&offset={total - 1}",
                headers=auth_header(alice_token),
            )
    ...
    def test_string_limit_rejected(self, client, alice_token):
        ...
        resp = client.get("/Product/_?limit=abc", headers=auth_header(alice_token))
        assert resp.status_code == 422
