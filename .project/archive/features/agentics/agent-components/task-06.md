# Create ntx-agent-live.js streaming activity view

**ID:** task-06 | **Wave:** 3 | **Depends on:** task-01

## Intent

ntx-agent-live is the 'window into the agent's mind' — a component that shows what an agent is currently doing with distinct visual rendering per event type. Unlike ntx-stream (which appends text) or ntx-chat (which is a conversational panel), this is a structured activity log view suitable for dashboards.

## Context

Agent components live at packages/n3tx-agents/src/n3tx_agents/static/components/. Currently only ntx-chat.js exists there. The new component should follow the same patterns.

Design decisions:
- Extends Component (from n3tx_core) not NTTStream — needs custom rendering per event type
- Uses HTTP.stream() directly for SSE (same as ntx-chat.js)
- Attributes: model, ref (entity address like 'agents/1'), method (default 'agentic_stream')
- The component renders a task input + send button at top, activity log below
- Each event type gets distinct visual treatment
- Activity log auto-scrolls
- Self-contained CSS in shadow DOM

Imports: HTTP from core/transport/HTTP.js, config from config.js, NTT from core/NTT.js (same as ntx-chat.js)

## Instructions

Create a new file at `/workspace/packages/n3tx-agents/src/n3tx_agents/static/components/ntx-agent-live.js`.
...
Web components use shadow DOM with inline styles via static styles property. customElements.define uses 'ntx-' prefix. Class names use NTT prefix (NTTAgentLive). Export both the class and the define call. CSS variables use existing theme tokens. HTML escaping via _esc() method. Schema loading via NTT.attach() callback pattern.

## Files

**Read:** - /workspace/packages/n3tx-agents/src/n3tx_agents/static/components/ntx-chat.js
- /workspace/packages/n3tx-core/src/n3tx_core/static/core/Component.js

**Modify:** none

**Create:** - /workspace/packages/n3tx-agents/src/n3tx_agents/static/components/ntx-agent-live.js
...
**Checks:**
- File exists at packages/n3tx-agents/src/n3tx_agents/static/components/ntx-agent-live.js
- Component handles all 5 event types: thinking, tool_call, tool_result, text, done
- tool_call shows card with spinner that tool_result updates by call_id
- Auto-scrolls on new entries
- Status badge shows idle/running/done/error
- CSS is self-contained in shadow DOM
